...
function singleWordTranslate(text){
  ...
  if (text.length === 1 && /[aeiou]/i.test(text)){
    return (text + "ay");
  }
  //tests to see if user input length is >1 and is a vowel and applies rule
  if (/[aeiou]/i.test(text[0]) && text.length > 1) {
    return(text + "way");
  }
  ...
  if (/[^aeiou]/i.test(text[0]) && /[aeiou]/i.test(text[1])){
    return(text.slice(1, ) + text[0]+ "ay");
  }
  //tests to see if a word starts with multiple consonants
  if (/[^aeiou]/i.test(text[0]) && /[^aeiou]/i.test(text[1]) && /[^0-9]/.test(text[0])) {
